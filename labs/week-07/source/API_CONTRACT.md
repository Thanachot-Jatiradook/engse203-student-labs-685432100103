...
| **ผู้จัดทำ** | ธนโชติ จาติระดุก 685432100103 |
...
**ทำอะไร** สร้างคำร้องใหม่ · เซิร์ฟเวอร์จะสร้าง `id` และตั้ง `status` เป็น `pending` ให้อัตโนมัติ
...
**ทำอะไร** เปลี่ยนสถานะการดำเนินงานของคำร้องตามรหัสที่ระบุ
...
*หมายเหตุ: เมื่อลบคำร้องสำเร็จแล้ว หากเรียก `GET /api/requests/:id` เส้นเดิมซ้ำ หรือเรียก `DELETE /api/requests/:id` ซ้ำอีกครั้ง เซิร์ฟเวอร์จะตอบกลับด้วย `404 Not Found`*
...
|---|---|---|:---:|
| 1.0.0 | 01/09/2026 | เวอร์ชันแรก — GET, POST, DELETE | — |
